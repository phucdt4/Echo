...
(function (factory) {
    ...
    FileInput.prototype = {
        ...
        _uploadBatch: function () {
            ...
            $.each(files, function (key, data) {
                if (!$h.isEmpty(files[key])) {
                    counter++;
                    var name = 'pic' + counter;
                    var value = data;
                    var filename = self.filenames[key];
                    self.formdata.append(name, value, filename);
                }
            });
            ...
            self.getFrames(' .kv-file-remove').each(function () {
                ...
                self._handler($el, 'click', function () {
                    ...
                    $frame.fadeOut('slow', function () {
                        ...
                        //var files = document.getElementById('kv-explorer').files;
                        //var temp = self.filestack.filter(function(val){ return val!==undefined; });
                        //console.log(temp);
        ...
        BindTable: function(jsondata, tableid) {/*Function used to convert the JSON array to Html Table*/
        ...
        BindTableHeader: function(jsondata, tableid) {/*Function used to get all column names from JSON and bind the html table header*/
            var columnSet = [];
            var headerTr$ = $('<tr/>');
            for (var i = 0; i < jsondata.length; i++) {
        ...
        upload: function () {
            ...
            for (var i = 0; i < temp.length; i++) {
                ...
                reader.onload = (function (theFile) {
                    var custom = {};
                    custom.name = theFile.name;
                    custom.filetype = theFile.type.indexOf('image')?'image':'document';
                    return function(e){
                        custom.data = e.target.result;
                        self.customData.push(custom);
                        console.log(self.customData.length,self.temp.length);
                        if (self.customData.length == self.temp.length)
                        {
                            console.log(self.customData);
                ...
                if (temp[i]) {
                    if (temp[i].type.indexOf('image') > -1)
                    {
                        reader.readAsDataURL(temp[i]);
                    }
                    else if (temp[i].type.indexOf('xlsx') > -1)
                    {
                        reader.readAsArrayBuffer(temp[i]);
                    }
                    else
                    {
    ...
    $.fn.fileinput.defaults = {
        ...
        uploadIcon: '<i class="glyphicon glyphicon-upload"></i>',
        uploadClass: 'btn btn-warning btn-secondary',
        uploadUrl: null,
        uploadUrlThumb: null,
        uploadAsync: true,
        uploadExtraData: {},
