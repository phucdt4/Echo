! function(e) {
    ...
    e.FileDialog = function(a) {
        var o = e.extend(e.FileDialog.defaults, a),
            t = e(["<div class='modal fade'>", "    <div class='modal-dialog'>", "        <div class='modal-content'>", "            <div class='modal-header'>", "                <button type='button' class='close' data-dismiss='modal'>", "                    <span aria-hidden='true'>&times;</span>", "                    <span class='sr-only'>", o.cancel_button, "                    </span>", "                </button>", "                <h4 class='modal-title'>", o.title, "                </h4>", "            </div>", "            <div class='modal-body text-center'>",
                "<label class='btn btn-primary'><span class='fa fa-file'></span>&nbsp;Browse Photos<input  type='file' style='display: none;' /></label>&nbsp;<label onclick='capturePhotoWithData()' class='btn btn-secondary'><span class='fa fa-camera'></span>&nbsp;Take a picture</label>", "                <div class='bfd-dropfield' style='display: none'>", "                    <div class='bfd-dropfield-inner'>", o.drag_message, "                    </div>", "                </div>", "                <div class='container-fluid bfd-files'>", "                </div>", "            </div>", "            <div class='modal-footer'>", "                <button type='button' class='btn btn-primary bfd-ok'>", o.ok_button, "                </button>", "                <button type='button' class='btn btn-danger bfd-cancel'", "                                data-dismiss='modal'>", o.cancel_button, "                </button>", "            </div>", "        </div>", "    </div>", "</div>"
            ].join("")),
            n = !1,
            r = e("input:file", t),
            i = e(".bfd-dropfield", t),
        ...
        var d = [],
            ...
            c = function(a) {
                ...
                i.onloadstart = function() {
                    ...
                    n.attr("aria-valuenow", a.loaded), n.css("width", o), e(".sr-only2", n).text(o)
                }, i.onload = function(e) {
                    console.log("On Load");
                    a.content = e.target.result, d.push(a), n.removeClass("active");
                    var id = a.name+a.size;
                    var id2 = 'prc'+a.name+a.size;
                    document.getElementById(id).width = "200";
                    document.getElementById(id).height = "200";
                    document.getElementById(id).src = a.content;
                    document.getElementById(id2).style.display = 'none';
                    //$(id).attr("src",a.content);
                    //n.attr("aria-valuenow", a.loaded), n.css("src", a.content), e(name, n).text(o)
                    console.log(a.name+a.size)
                };
                var s = e(["<div class='col-xs-7 col-sm-4 bfd-info'>", "    <span class='fa fa-remove bfd-remove'></span>&nbsp;", "<span class='fa fa-file'></span>&nbsp;" + a.name, "</div>",
                "<div class='col-xs-5 col-sm-8 bfd-progress'>",
                "<img id='", a.name+a.size ,"' src='", a.content ,"'/>",
                "<div id='prc", a.name+a.size ,"' class='progress'>", "        <div class='progress-bar progress-bar-striped active' role='progressbar'", "            aria-valuenow='0' aria-valuemin='0' aria-valuemax='" + a.size + "'>", "<span class='sr-only2'>0%</span>", "</div>", "</div>", "</div>"].join(""));
